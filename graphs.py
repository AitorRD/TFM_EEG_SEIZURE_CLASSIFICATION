...
# Selected channels for multimodal visualization (frontal, central, temporal, parietal, occipital)
selected_channels = ["EEG Fp1", "EEG F3", "EEG C3", "EEG T3", "EEG P3", "EEG O1",
...
def plot_raw_eeg_traces(df, file_name, save_dir, duration_minutes=3):
    """
    Plot raw EEG signals from selected channels during preictal and ictal periods.
    ...
    print(f"Saved: {save_path} (Duration: {duration_minutes} min, Seizure onset: {seizure_onset_time:.1f}s)")
        ...
        if file.endswith("clipped.csv"):
            ...
            try:
                ...
                if "Seizure" in df.columns:
                    ...
                    # 2. Generate raw EEG traces (multimodal)
